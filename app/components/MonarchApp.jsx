...
function Chat() {
  ...
  const [msgs,setMsgs] = useState([{from:"bot",text:"Welcome to Monarch Blends. Ask me about our products, dealer programme, or machinery rights."}]);
      ...
      {open && <div style={{background:"#111",border:`1px solid rgba(184,134,11,.3)`,borderTop:`2px solid ${G}`,marginBottom:10,borderRadius:2}}>
        ...
        <div style={{display:"flex",borderTop:`1px solid ${BR}`}}>
          <input value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()}
            placeholder="Ask about Monarch Blends…"
...
// ─── Marquee ──────────────────────────────────────────────────
function Marquee() {
  const items=["Hand-Picked Tobacco","Formulated in North America","Made in India 🇮🇳","2+ Years R&D","100% Compliant","Exclusive Machinery Rights","Pan India Distribution","Customisable Tubes"];
  return (
...
function Admin({onClose}) {
  ...
  useEffect(()=>{
    getDoc(doc(db,"settings","contact")).then(d=>{if(d.exists())setCfg(d.data().value);});
  },[]);
  ...
  const TABS=[{k:"jobs",l:"Jobs"},{k:"services",l:"Services"},{k:"contacts",l:"Enquiries"},{k:"settings",l:"Settings"}];
...
export default function App() {
  ...
  useEffect(()=>{
    getDoc(doc(db,"settings","contact")).then(d=>{if(d.exists())setSettings(d.data().value);});
  },[page]);
  ...
  if(admin) return <Admin onClose={()=>setAdmin(false)}/>;

  const NAV=[{k:"home",l:"Home"},{k:"about",l:"About"},{k:"services",l:"Services"},{k:"dealer",l:"Dealer / Distributor"},{k:"careers",l:"Careers"},{k:"contact",l:"Contact"}];
  ...
  return (
    <div style={{fontFamily:"Inter,sans-serif",background:"#060606",color:IV,minHeight:"100vh",overflowX:"hidden"}}>

      {/* ── NAV ── */}
      <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:500,height:68,
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={()=>go("home")}>
          ...
          <div>
            <GoldText sz={15} ls={3}>MONARCH BLENDS</GoldText>
            <div style={{fontSize:8,color:"rgba(248,244,236,.4)",letterSpacing:2,textTransform:"uppercase",marginTop:2}}>Shree Siddheshwari Enterprise Pvt. Ltd.</div>
          </div>
        </div>
      ...
      {page==="home"&&<>
          ...
          <div style={{position:"relative",zIndex:2,textAlign:"center",maxWidth:900,padding:"0 24px"}}>
            ...
            <div style={{position:"relative",width:190,height:190,margin:"0 auto 36px"}}>
              ...
              <div style={{position:"absolute",inset:9,borderRadius:"50%",overflow:"hidden",
                ...
                <img src={LOGO} alt="Monarch Blends" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
              </div>
            </div>
            <div style={{fontFamily:"Cinzel,serif",fontSize:10,letterSpacing:5,textTransform:"uppercase",
              ...
              North American Formula ◆ Made in India ◆ Hand-Picked Tobacco
            </div>
            ...
            <p style={{fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",
              ...
              The Sovereign Standard in Premium Tobacco<br/>By Shree Siddheshwari Enterprise Pvt. Ltd.
            </p>
            <div style={{display:"flex",alignItems:"center",gap:18,justifyContent:"center",marginBottom:34}}>
              ...
              <span style={{fontFamily:"Cinzel,serif",fontSize:9,letterSpacing:3,color:G}}>Est. Gujarat, India</span>
              <div style={{width:60,height:1,background:`linear-gradient(90deg,${G},transparent)`}}/>
            </div>
            <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
              <Btn onClick={()=>go("dealer")}>Become a Partner</Btn>
              <Btn outline onClick={()=>go("about")}>Our Heritage</Btn>
            </div>
          </div>
        </section>
        ...
        <section style={{padding:"88px 60px",maxWidth:1300,margin:"0 auto"}}>
          <SecEye>The Monarch Difference</SecEye>
          <SecTitle>Crafted Without Compromise</SecTitle>
        ...
        <div style={{background:"#111",borderTop:"1px solid rgba(184,134,11,.15)",borderBottom:"1px solid rgba(184,134,11,.15)",padding:"88px 60px"}}>
          <div style={{textAlign:"center",maxWidth:1300,margin:"0 auto"}}>
            <SecEye>From Seed to Sovereign</SecEye>
            <SecTitle mb={0}>The Monarch Story</SecTitle>
          </div>
      ...
      {/* ═══════════ ABOUT ═══════════ */}
      {page==="about"&&<>
        <PageHero eye="Our Heritage" h1="About Monarch Blends" sub="Global expertise. Indian craft. Royal ambition."/>
        <div style={{padding:"80px 60px",maxWidth:1300,margin:"0 auto"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:72,alignItems:"start"}}>
            <div>
              <SecTitle mb={26}>The Making of a Sovereign Brand</SecTitle>
              ...
              ].map((p,i)=><p key={i} style={{fontSize:14,color:MU,lineHeight:1.9,marginBottom:16}}>{p}</p>)}
              <blockquote style={{fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",fontSize:18,color:GP,
                ...
                "Our formula was not rushed. Over two years of research and development — testing, refining, perfecting — went into every aspect of Monarch Blends before a single product reached a retailer."
              </blockquote>
            </div>
            <div style={{background:"#111",border:`1px solid ${BR}`,borderTop:`2px solid ${G}`,padding:42}}>
              <GoldText sz={16} ls={2} style={{display:"block",marginBottom:30}}>Our Core Values</GoldText>
      ...
      {/* ═══════════ SERVICES ═══════════ */}
      {page==="services"&&<>
        <PageHero eye="What We Do" h1="Our Services" sub="From leaf to distribution — a complete sovereign ecosystem"/>
        <section style={{padding:"80px 60px",maxWidth:1300,margin:"0 auto"}}>
          <SecEye>Core Capabilities</SecEye>
          <SecTitle>The Full Monarch Offering</SecTitle>
      ...
      {/* ═══════════ DEALER ═══════════ */}
      {page==="dealer"&&<>
        <PageHero eye="Business Opportunity" h1="Partner with Monarch Blends" sub="Join the sovereign network across India and beyond"/>

        {/* TIERS */}
        <section style={{padding:"80px 60px",maxWidth:1300,margin:"0 auto"}}>
          <SecEye>Partnership Tiers</SecEye>
          <SecTitle>Choose Your Level</SecTitle>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:2,background:"rgba(184,134,11,.15)"}}>
            {[
              {feat:false,badge:"Retail Dealer",icon:"🏪",title:"Retail Dealer",desc:"Sell Monarch Blends directly through your outlet. The perfect entry into one of India's fastest-growing premium tobacco brands.",
                items:["Low minimum order quantity","Attractive retail margins","POS & branding materials","Consistent supply guarantee","Promotional campaign support"]},
              {feat:true,badge:"Most Popular",icon:"🏢",title:"Area Distributor",desc:"Distribute across your district or region. Exclusive area rights, higher margins, and full commercial support.",
                items:["Exclusive territory rights","Superior volume margins","Dedicated sales support team","Priority stock allocation","Co-branded marketing materials"]},
              {feat:false,badge:"State Master",icon:"🌐",title:"State Distributor",desc:"Become the master distributor for your entire state. Build a full dealer network with state-wide exclusivity.",
                items:["Full state-level exclusivity","Best-in-class margins","Direct factory pricing","Full marketing ecosystem","Personal relationship manager"]},
            ].map(t=>(
        ...
        <div style={{background:"#111",borderTop:"1px solid rgba(184,134,11,.15)",borderBottom:"1px solid rgba(184,134,11,.15)",padding:"80px 60px"}}>
          <div style={{display:"grid",gridTemplateColumns:"1.2fr 1fr",gap:72,maxWidth:1300,margin:"0 auto",alignItems:"start"}}>
            <div>
              <SecEye>Exclusive Rights</SecEye>
              <SecTitle mb={22}>Machinery & Tube — Master Rights for India</SecTitle>
              ...
              <div style={{display:"inline-flex",alignItems:"center",gap:10,border:`1px solid ${G}`,
                ...
                ◆ Exclusive India & Neighbouring Country Rights
              </div>
            </div>
        ...
        {/* BENEFITS */}
        <section style={{padding:"80px 60px",maxWidth:1300,margin:"0 auto"}}>
          <SecEye>Partner Benefits</SecEye>
          <SecTitle>The Monarch Partner Advantage</SecTitle>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:1,background:"rgba(184,134,11,.15)"}}>
        ...
        <div style={{background:"#000",borderTop:"1px solid rgba(184,134,11,.15)",padding:"80px 60px",textAlign:"center"}}>
          <SecTitle>Ready to Join the Monarch Network?</SecTitle>
          <p style={{fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",fontSize:18,color:MU,marginBottom:32,maxWidth:600,margin:"0 auto 32px"}}>
            Contact us today and our partnership team will respond within 2 business days.
          </p>
          <Btn onClick={()=>go("contact")}>Apply for Partnership</Btn>
        </div>
      </>}

      {/* ═══════════ CAREERS ═══════════ */}
      {page==="careers"&&<>
        <PageHero eye="Join Our Team" h1="Careers at Monarch Blends" sub="Build something sovereign from the ground up"/>
        <section style={{padding:"80px 60px",maxWidth:1000,margin:"0 auto"}}>
          <p style={{fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",fontSize:20,color:MU,
            ...
            "We are a young, ambitious brand — looking for people who share that ambition. At Monarch Blends, you are not joining a legacy. You are helping build one."
          </p>
          <SecEye>Open Positions</SecEye>
          <SecTitle mb={28}>Current Openings</SecTitle>
          <div style={{borderTop:"1px solid rgba(184,134,11,.12)"}}>
        ...
        <div style={{background:"#111",borderTop:"1px solid rgba(184,134,11,.15)",padding:"70px 60px"}}>
          <div style={{maxWidth:1300,margin:"0 auto"}}>
            <SecEye>Benefits</SecEye>
            <SecTitle mb={32}>What We Offer</SecTitle>
      ...
      {/* ═══════════ CONTACT ═══════════ */}
      {page==="contact"&&<>
        <PageHero eye="Get In Touch" h1="Contact Us" sub="Dealers, partners, machinery buyers, job seekers — we welcome you"/>
        <div style={{padding:"80px 60px",maxWidth:1300,margin:"0 auto"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1.6fr",gap:72}}>
            <div>
              <GoldText sz={22} ls={2} style={{display:"block",marginBottom:36}}>Reach Out</GoldText>
              {[{ico:"📍",l:"Address",v:`Monarch Blends
Shree Siddheshwari Enterprise Pvt. Ltd.
${settings.address}`},
            ...
            <div style={{background:"#111",border:`1px solid ${BR}`,borderTop:`2px solid ${G}`,padding:42}}>
              <GoldText sz={18} ls={2} style={{display:"block",marginBottom:28}}>Send a Message</GoldText>
              {sent?(
                <div style={{textAlign:"center",padding:"40px 20px",animation:"fadeIn .5s ease"}}>
                  <div style={{fontSize:48,marginBottom:16}}>👑</div>
                  <GoldText sz={20} ls={1} style={{display:"block",marginBottom:10}}>Message Received</GoldText>
                  <p style={{fontSize:13,color:MU,marginBottom:24}}>Thank you for reaching out. We will respond within 1–2 business days.</p>
                ...
                <>
                  ...
                  <Btn full onClick={submitForm}>{sending?"Saving to Firebase…":"Send Message ◆"}</Btn>
                </>
              )}
            </div>
          </div>
      ...
      <footer style={{background:"#000",borderTop:"1px solid rgba(184,134,11,.15)",padding:"0 60px 32px"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:20,maxWidth:1300,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            ...
            <div>
              <GoldText sz={14} ls={3}>MONARCH BLENDS</GoldText>
              <div style={{fontSize:8,color:"rgba(248,244,236,.35)",letterSpacing:2,textTransform:"uppercase",marginTop:2}}>Shree Siddheshwari Enterprise Pvt. Ltd.</div>
            </div>
          </div>
          <div style={{fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",fontSize:13,color:"rgba(248,244,236,.3)",textAlign:"center",lineHeight:1.7}}>
            Hand-Picked Tobacco ◆ Formulated in North America ◆ Made in India
          </div>
          <div style={{fontSize:11,color:"rgba(248,244,236,.22)",textAlign:"right",letterSpacing:1,lineHeight:1.8}}>
            © 2025 Shree Siddheshwari Enterprise Pvt. Ltd.<br/>All Rights Reserved · Gujarat, India
          </div>
        </div>
      </footer>
